...
void execute_pipeline(pipeline apipe) {
    ...
    if (pipeline_is_empty(apipe)) {                                                 // caso: pipeline vacío (no hay nada que ejecutar)
        return;
    }
    
    if (builtin_alone(apipe)) {                                                     // caso: pipeline con un solo comando interno, se ejecuta en el proceso padre. 
    ...
    for (unsigned int i = 0; i < apipe_length; i++) {
        ...
        if (i < apipe_length - 1) {                                                 // si no es el último comando crea un nuevo pipe
            pipe(pipe_filedes);
        }

        int pid = fork();                                                           // se crea un nuevo proceso hijo
        ...
        if (have_temp && scommand_get_redir_in(cmd) == NULL) {                      // si no es el primer comando conecto stdin con la salida del pipe anterior
            dup2(temp[0], STDIN_FILENO);
            close(temp[0]);
            close(temp[1]);
        }

        if (i < apipe_length - 1  && scommand_get_redir_out(cmd) == NULL) {         // si no es el ultimo comando conecto stdout al pipe actual
